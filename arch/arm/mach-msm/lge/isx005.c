	...
	for (i = 0; i < num_of_items_in_table; ++i) {
	 if (reg_conf_tbl->register_address == 0xdddd) {  // sensor register refresh polling
	  do {
 	  ...
 	  //printk(KERN_ERR "table polling 0x%x\n", temp);
...
printk(KERN_ERR "pll addr : 0x%x, value : 0x%x, length : 0x%x",
        isx005_regs.init_reg_settings[i].register_address,
        isx005_regs.init_reg_settings[i].register_value,
        isx005_regs.init_reg_settings[i].register_length);
...
int isx005_reg_tuning(void *data)
{
	...
	for (i = 0; i < isx005_regs.tuning_reg_settings_size; ++i) {
	 if ( isx005_regs.tuning_reg_settings[i].register_address == 0xdddd ) {  /* delay */
	   mdelay(300);
	 } else {
 		rc = isx005_i2c_write(isx005_client->addr,
...
/*
	unsigned short pixel, frame;

	isx005_i2c_read(isx005_client->addr, 0x3C68, &pixel, WORD_LEN);
	isx005_i2c_read(isx005_client->addr, 0x3C66, &frame, WORD_LEN);
	printk(KERN_INFO "before preview cmd [pixel : 0x%x, frame : 0x%x]\n", pixel, frame);
*/
 ...
 do {
  isx005_i2c_write(isx005_client->addr, 0x098E, 0x8400, WORD_LEN);
  isx005_i2c_read(isx005_client->addr, 0x0990, &temp, WORD_LEN);
  //printk(KERN_ERR "preview polling 0x%x", temp);

  /*
  isx005_i2c_read(isx005_client->addr, 0x3C68, &pixel, WORD_LEN);
 	isx005_i2c_read(isx005_client->addr, 0x3C66, &frame, WORD_LEN);
 	printk(KERN_INFO "doing preview polling [pixel : 0x%x, frame : 0x%x]\n", pixel, frame);
 	*/
 } while( temp & 0x0001 );

 //printk(KERN_ERR "preview polling end 0x%x\n", temp);

 /*
 isx005_i2c_read(isx005_client->addr, 0x3C68, &pixel, WORD_LEN);
	isx005_i2c_read(isx005_client->addr, 0x3C66, &frame, WORD_LEN);
	printk(KERN_INFO "after preview cmd [pixel : 0x%x, frame : 0x%x]\n", pixel, frame);
	*/
 ...
 /*
	for (i = 0; i < 300; i++) {
 	...
 	isx005_i2c_write(isx005_client->addr, 0x098E, 0x8401, WORD_LEN);
  isx005_i2c_read(isx005_client->addr, 0x0990, &cm_changed_status, WORD_LEN);

		if (cm_changed_status == 0x0003) {
		 printk(KERN_ERR "preview polling 0x%x\n", cm_changed_status);
		 //mdelay(300);
			break;
		} else {
		  printk(KERN_ERR "preview sleep!!!!!!!! 0x%x\n", cm_changed_status);
			 msleep(10);
		}
	}
	*/
...
static int isx005_reg_snapshot(void)
{
 ...
 do {
  isx005_i2c_write(isx005_client->addr, 0x098E, 0x8400, WORD_LEN);
  isx005_i2c_read(isx005_client->addr, 0x0990, &temp, WORD_LEN);
  //printk(KERN_ERR "snapshot polling 0x%x\n", temp);
 } while( temp & 0x0002 );

 //printk(KERN_ERR "snapshot polling end 0x%x\n", temp);
 ...
 do {
  ...
  printk(KERN_ERR "snapshot status 0x%x\n", temp);
 } while( temp != 0x0007 );

 printk(KERN_ERR "snapshot status end 0x%x\n", temp);
 */
...
static int isx005_set_effect(int effect)
{
 ...
 do {
  isx005_i2c_write(isx005_client->addr, 0x098E, 0x8400, WORD_LEN);
  isx005_i2c_read(isx005_client->addr, 0x0990, &temp, WORD_LEN);
  //printk(KERN_ERR "effect polling 0x%x\n", temp);
...
static int32_t isx005_set_brightness(int8_t brightness)
{
 int rc = 0;
	switch (brightness) {
	case 0: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0xC300, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 1: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0xCF00, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 2: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0xDB00, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 3: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0xE700, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 4: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0xF300, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 5: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0x0000, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 6: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0x0C00, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 7: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0x1800, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 8: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0x2400, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 9: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0x3000, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	case 10: {
  rc = isx005_i2c_write(isx005_client->addr, 0x337E, 0x3C00, WORD_LEN);
		if (rc < 0)
			return rc;
	...
	for (i = 0; i < isx005_regs.tuning_reg_settings_size; ++i) {
	 if ( isx005_regs.tuning_reg_settings[i].register_address == 0xdddd ) {
	  do {
 	  isx005_i2c_write(isx005_client->addr, 0x098E, 0x8400, WORD_LEN);
 	  isx005_i2c_read(isx005_client->addr, 0x0990, &temp, WORD_LEN);
 	  //printk(KERN_ERR "init polling 0x%x\n", temp);
 	 } while( temp & 0x0006 );
	  //printk(KERN_ERR "init polling end 0x%x\n", temp);
	 } else {
   	rc = isx005_i2c_write(isx005_client->addr,
   		isx005_regs.tuning_reg_settings[i].register_address,
   		isx005_regs.tuning_reg_settings[i].register_value,
...
int isx005_sensor_config(void __user *argp)
{
 ...
 case CFG_CHECK_AF_DONE:
	...
	case CFG_SET_WB: {
	 printk(KERN_INFO "white balance in\n");
	...
	case CFG_SET_BRIGHTNESS: {
	 printk(KERN_INFO "brightness in\n");
