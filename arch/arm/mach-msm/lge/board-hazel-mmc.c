/* arch/arm/mach-msm/lge/board-hazel-mmc.c
...
#ifdef CONFIG_MMC_MSM_CARD_HW_DETECTION
static unsigned int hazel_sdcc_slot_status(struct device *dev)
{
	printk(KERN_ERR "[LGE] sdcc_slot_status: card detect - %d\n", gpio_get_value(GPIO_SD_DETECT_N));	//LGE_UPDATE FS
	#if 0 // LGE_CHANGE
...
/* LGE_CHANGE_S [jisung.yang@lge.com] 2010-04-24, BCM4325 control */
	...
	gpio_tlmm_config(GPIO_CFG(CONFIG_BCM4329_GPIO_WL_REGON, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE);
	gpio_configure(CONFIG_BCM4329_GPIO_WL_REGON, GPIOF_DRIVE_OUTPUT);
	gpio_set_value(CONFIG_BCM4329_GPIO_WL_REGON, 0);
#endif
	
	gpio_tlmm_config(GPIO_CFG(CONFIG_BCM4329_GPIO_WL_RESET, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE);
	gpio_configure(CONFIG_BCM4329_GPIO_WL_RESET, GPIOF_DRIVE_OUTPUT);
	gpio_set_value(CONFIG_BCM4329_GPIO_WL_RESET, 0);
	
	gpio_tlmm_config(GPIO_CFG(CONFIG_BCM4329_GPIO_WL_HOSTWAKEUP, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), GPIO_ENABLE);
	gpio_configure(CONFIG_BCM4329_GPIO_WL_HOSTWAKEUP, GPIOF_INPUT);
