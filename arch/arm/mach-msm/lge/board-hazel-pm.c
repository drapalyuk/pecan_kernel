/* arch/arm/mach-msm/lge/board-hazel-pm.c
...
int hazel_pwrsink_suspend_noirq(struct device *dev)
{
...
int hazel_pwrsink_resume_noirq(struct device *dev)
{
...
static struct dev_pm_ops hazel_pwrsink_data = {
	.suspend_noirq = hazel_pwrsink_suspend_noirq,
	.resume_noirq = hazel_pwrsink_resume_noirq,
};

static struct platform_device hazel_pwrsink_device = {
	.name = "lge-pwrsink",
	.id = -1,
	.dev = {
		.platform_data = &hazel_pwrsink_data,
	},
};
...
{
	lcd_bl_power_state = BL_POWER_RESUME;
	platform_device_register(&hazel_pwrsink_device);
}
