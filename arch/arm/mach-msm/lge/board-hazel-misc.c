/* arch/arm/mach-msm/lge/board-hazel-misc.c
...
/* hazel Board Vibrator Functions for Android Vibrator Driver */
#define VIBE_IC_VOLTAGE			2200 /* Hazel REV_F : motor ic voltage is changed to 2.2V by bongkyu.kim */
...
static void hazel_desk_dock_detect_callback(int state)
{
	int ret;
...
static int hazel_register_callback(void)
{
	rpc_server_hs_register_callback(hazel_desk_dock_detect_callback);

	return 0;
}

static int hazel_gpio_carkit_work_func(void)
{
	return DOCK_STATE_UNDOCKED;
}

static char *hazel_gpio_carkit_print_state(int state)
{
	return dock_state_string[state];
}

static int hazel_gpio_carkit_sysfs_store(const char *buf, size_t size)
{
	int state;
...
static unsigned hazel_carkit_gpios[] = {
};

static struct lge_gpio_switch_platform_data hazel_carkit_data = {
	.name = "dock",
	.gpios = hazel_carkit_gpios,
	.num_gpios = ARRAY_SIZE(hazel_carkit_gpios),
	.irqflags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
	.wakeup_flag = 1,
	.work_func = hazel_gpio_carkit_work_func,
	.print_state = hazel_gpio_carkit_print_state,
	.sysfs_store = hazel_gpio_carkit_sysfs_store,
	.additional_init = hazel_register_callback,
};

static struct platform_device hazel_carkit_device = {
	.name = "lge-switch-gpio",
	.id = 0,
	.dev = {
		.platform_data = &hazel_carkit_data,
	},
};
...
static struct platform_device msm_device_pmic_key_leds = {
	.name = "pmic-key-leds",
	.id = -1,
	.dev.platform_data = "keyboard-backlight",
};

int hazel_vibrator_power_set(int enable)
{
	static int is_enabled = 0;
	struct device *dev = hazel_backlight_dev();
	...
	if (enable) {		
		...
		/* Hazel REV_F : motor ic voltage is changed to 2.2V by bongkyu.kim */
		if (lge_bd_rev <= LGE_REV_D) {
			if (aat28xx_ldo_set_level(dev, 1, 3300) < 0) {
...
int hazel_vibrator_pwm_set(int enable, int amp)
{
...
int hazel_vibrator_ic_enable_set(int enable)
{
	/* nothing to do, thunder does not using Motor Enable pin */
	return 0;
}

static struct android_vibrator_platform_data hazel_vibrator_data = {
	.enable_status = 0,	
	.power_set = hazel_vibrator_power_set,
	.pwm_set = hazel_vibrator_pwm_set,
	.ic_enable_set = hazel_vibrator_ic_enable_set,
	.amp_value = 115,
};
...
static struct platform_device android_vibrator_device = {
	...
	.id = -1,
	.dev = {
		.platform_data = &hazel_vibrator_data,
	},
};
...
static int hazel_gpio_earsense_work_func(void)
{
	int state;
	int gpio_value;
	...
	if(gpio_value == EAR_INJECT)
	{
		printk(KERN_INFO"%s: ear sense delay : 300\n", __func__);
		mdelay(100);
		gpio_value = gpio_get_value(GPIO_EAR_SENSE);
		printk(KERN_INFO"%s: ear sense detected2 : %s\n", __func__, 
			gpio_value?"injected":"ejected");
	}
...
static char *hazel_gpio_earsense_print_state(int state)
{
	return ear_state_string[state];
}

static int hazel_gpio_earsense_sysfs_store(const char *buf, size_t size)
{
	int state;
...
static unsigned hazel_earsense_gpios[] = {
	GPIO_EAR_SENSE,
};

static struct lge_gpio_switch_platform_data hazel_earsense_data = {
	.name = "h2w",
	.gpios = hazel_earsense_gpios,
	.num_gpios = ARRAY_SIZE(hazel_earsense_gpios),
	.irqflags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
	.wakeup_flag = 1,
	.work_func = hazel_gpio_earsense_work_func,
	.print_state = hazel_gpio_earsense_print_state,
	.sysfs_store = hazel_gpio_earsense_sysfs_store,
};

static struct platform_device hazel_earsense_device = {
	.name   = "lge-switch-gpio",
	.id = 1,
	.dev = {
		.platform_data = &hazel_earsense_data,
	},
};

/* misc platform devices */
static struct platform_device *hazel_misc_devices[] __initdata = {
	&msm_batt_device,
	&android_vibrator_device,
	&hazel_carkit_device,
	&hazel_earsense_device,
	&msm_device_pmic_leds,
	&msm_device_pmic_key_leds,
};

/* main interface */
void __init lge_add_misc_devices(void)
{
	platform_add_devices(hazel_misc_devices, ARRAY_SIZE(hazel_misc_devices));
}
