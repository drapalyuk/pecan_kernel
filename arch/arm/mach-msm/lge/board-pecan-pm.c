...
int pecan_pwrsink_suspend_noirq(struct device *dev)
{
...
int pecan_pwrsink_resume_noirq(struct device *dev)
{
...
static struct dev_pm_ops pecan_pwrsink_data = {
	.suspend_noirq = pecan_pwrsink_suspend_noirq,
	.resume_noirq = pecan_pwrsink_resume_noirq,
};

static struct platform_device pecan_pwrsink_device = {
	.name = "lge-pwrsink",
	.id = -1,
	.dev = {
		.platform_data = &pecan_pwrsink_data,
	},
};
...
{
	lcd_bl_power_state = BL_POWER_RESUME;
	platform_device_register(&pecan_pwrsink_device);
}
