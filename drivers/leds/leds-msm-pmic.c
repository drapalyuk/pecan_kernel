...
#ifdef CONFIG_MACH_MSM7X27_HAZEL
	if (lge_bd_rev == LGE_REV_C) {
		/* TODO: hazel rev_c keypad led using VIB_DRV_N 
		 * it should be modified in next revision by bongkyu.kim */
		...
		if (value == 0)
			...
			pmic_vib_mot_set_volt(1200 + ((19 * value / 255) * 100));
	} else {
		pmic_set_led_intensity(LED_LCD, value / TUNED_MAX_KEYPAD_BL_LEVEL);
	}
#else
