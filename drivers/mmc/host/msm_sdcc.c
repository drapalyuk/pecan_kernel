...
//LGE_UPDATE_S BCPARK
#if 1
#define MSMSDCC_MAX_DETECT_DEBOUNCE		5
...
//LGE_UPDATE_S BCPARK
#if 1
	if(strcmp("mmc0", mmc_hostname(mmc)) == 0)
	{
	...
	{
		pr_info("[LGE] %s - wifi is detected\n", mmc_hostname(mmc));		
		status = host->plat->status(mmc_dev(host->mmc));
	}
...
//LGE_UPDATE_E BCPARK
		...
		host->oldstat = status;	//LGE_UPDATE BCPARK
	}
...
//#ifdef CONFIG_LGE_BCM432X_PATCH
			if (host->plat->status_irq == MSM_GPIO_TO_INT(CONFIG_BCM4329_GPIO_WL_RESET)) {
...
/* LGE_CHANGE_S, [jisung.yang@lge.com], 2010-04-24, <never sleep policy - host wakeup> */
		...
		else if (host->plat->status_irq == gpio_to_irq(CONFIG_BCM4329_GPIO_WL_RESET)) {
...
#else
		if ((mmc->pm_flags & MMC_PM_WAKE_SDIO_IRQ) && mmc->card &&
				mmc->card->type == MMC_TYPE_SDIO && host->plat->status_irq != gpio_to_irq(CONFIG_BCM4329_GPIO_WL_RESET)) {
#endif			
...
#else
			if (mmc->card && mmc->card->type == MMC_TYPE_SDIO && host->plat->status_irq != gpio_to_irq(CONFIG_BCM4329_GPIO_WL_RESET)) {
#endif			
/* LGE_CHANGE_E, [jisung.yang@lge.com], 2010-05-04, <do not do for wifi> */
